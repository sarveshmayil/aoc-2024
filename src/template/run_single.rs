...
use super::timings::{Timing};

pub fn run_single(day: Day, is_release: bool, is_timed: bool) -> Option<Timing> {
    ...
    if output.is_empty() {
        ...
        timing = child_commands::parse_exec_time(&output, day);
    }

    if is_timed {
        let total_millis = timing.total_nanos / 1_000_000_f64;
